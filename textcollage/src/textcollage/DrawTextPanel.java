...
// where the user inputs the string that will be added to the canvas

private JTextField input;

// for letting the user select files

private SimpleFileChooser fileChooser;

// a menu bar with command that affect this panel

private JMenuBar menuBar;

// a listener that responds whenever the user selects a menu command

private MenuHandler menuHandler;

// the "Remove Item" command from the edit menu
...
input = new JTextField("Let's Create Something Cool!", 40);
...
// Default is null, meaning default color of the canvas (black).
...
// Default is 0, meaning text is not at all transparent.

// s.setBackground(Color.BLUE);

// Default is null, meaning don't draw a background area.

// s.setBackgroundTransparency(0.7);

// Default is 0, meaning background is not transparent.
...
// for making keyboard accelerators for menu commands
...
// undoItem was created in the constructor
...
bw.write(dti.getString()+"~"+dti.getX()+"~"

+dti.getY()+"~"+dti.getTextColor().getRGB()

+"~"+dti.getRotationAngle());
...
dti.setTextColor(new Color(Integer.parseInt(element.split("~")[3]))); dti.setRotationAngle(Double.parseDouble(element.split("~")[4]));
...
// Because the image is not available,

//I will make a new BufferedImage and

// draw the same data to the BufferedImage as is

//shown in the panel.

// A BufferedImage is an image that is stored in memory,

//not on the screen.

// There is a convenient method for writing a BufferedImage
...
throw new Exception("PNG format not supported (this"
